...
pub fn load_track(
    ...
    let http_protocol = if cfg!(feature = "tls") {
        "https"
    } else {
        "http"
    };

    let url = format!("{http_protocol}://{address}/v4/loadtracks?identifier={identifier}");
...
pub fn decode_track(
    ...
    let url = format!("{http_protocol}://{address}/v4/decodetrack?encodedTrack={identifier}");
